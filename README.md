...
- **CI/CD**: GitHub Actions 6개 워크플로 (lint+test, BE/FE/Jobs/Airflow VM 자동 배포, AI 코드리뷰)
- **Airflow 오케스트레이션**: 수집+인덱싱+AWS 동기화 DAG (매일 02:00 KST), DataSync 전용/평가/QA 생성 DAG (수동)
- **멀티클라우드 동기화**: FAISS 인덱스 GCS→S3 동기화 (AWS DataSync) + GitHub Actions 배포 트리거
...
Airflow VM (e2-standard-2)
  │
  ├── DAG 1: 수집+인덱싱+AWS 동기화 (매일 02:00 KST)
  │     collect → index_build → cloud_run_restart (GCP)
  │                           → datasync (GCS→S3) → aws_deploy_trigger
  │
  ├── DAG 2: DataSync 전용 (수동 트리거)
  │     datasync (GCS→S3) → aws_deploy_trigger
  │
  ├── DAG 3: 평가 (수동 트리거)
...
dags/                     # ✅ Airflow DAGs 4개 (수집+인덱싱+AWS동기화, DataSync전용, 평가, QA생성)
scripts/
...
GitHub Actions 6개 워크플로로 모노레포 경로 필터를 적용해 변경된 서비스만 배포한다.
...
| `pr-agent.yml` | PR 생성/동기화 | Qodo PR-Agent + Gemini AI 코드리뷰 |
